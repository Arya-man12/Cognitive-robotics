...
void applyRampedTargets(int targetL, int targetR) {
  ...
  while (curLeftSpeed != targetL || curRightSpeed != targetR) {
    if (curLeftSpeed < targetL) curLeftSpeed = min(curLeftSpeed + RAMP_STEP, targetL);
    else if (curLeftSpeed > targetL) curLeftSpeed = max(curLeftSpeed - RAMP_STEP, targetL);

    if (curRightSpeed < targetR) curRightSpeed = min(curRightSpeed + RAMP_STEP, targetR);
    else if (curRightSpeed > targetR) curRightSpeed = max(curRightSpeed - RAMP_STEP, targetR);
...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    if (in == '\n') {
      ...
      inputLine += in;
      // protect line length
      if (inputLine.length() > 10) inputLine = "";
    }
  }
